...
class DoorrolFinder:
    ...
    def find_candidates(self, rows: list[dict]) -> list[CandidatePair]:
        ...
        for row in rows:
            ...
            key = (
                d,
                _asset_rollup_price_insensitive(row.get("asset_rollup")),
                _asset_rollup_price_insensitive(row.get("asset_detail")),
                qty_abs,
            )
    ...
    def _score_pair(self, a: dict, b: dict) -> tuple[float, list[str]]:
        ...
        if _clean(a.get("asset_rollup")).upper() == _clean(b.get("asset_rollup")).upper():
            score += 3.0
            notes.append("zelfde asset_rollup")
        else:
            score += 2.0
            notes.append("asset_rollup verschilt (prijsverschil toegestaan)")

        if _clean(a.get("asset_detail")).upper() == _clean(b.get("asset_detail")).upper():
            score += 2.0
            notes.append("zelfde asset_detail")
        else:
            score += 1.0
            notes.append("asset_detail verschilt (prijsverschil toegestaan)")
        return score, notes
...
class ReviewWindow(QWidget):
    ...
    def render(self) -> None:
        ...
        self.lbl_meta.setText(
            "Regels: zelfde dag + asset/asset_detail (prijs mag verschillen) + abs qty, tegengesteld, order_id/origin nog leeg."
        )
        for r, row in enumerate([p.left, p.right]):
            ...
            self.tbl.setItem(r, 5, self._item(row.get("side")))
            self.tbl.setItem(r, 6, self._item(row.get("qty")))
            self.tbl.setItem(r, 7, self._item(f"{p.score:.2f}" if r == 0 else ""))
            self.tbl.setItem(r, 8, self._item(", ".join(p.notes) if r == 0 else ""))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Vind en review doorrol-sprintertransacties zonder order_id/transactie_oorsprong."
    )
