...
class DoorrolFinder:
    ...
    def find_candidates(self, rows: list[dict]) -> list[CandidatePair]:
        ...
        for row in rows:
            ...
            if qty_abs <= 0:
                continue
            key = (d, _clean(row.get("asset_rollup")).upper(), qty_abs)
            ...
            for b in sorted(buys, key=lambda x: int(x.get("Id"))):
                ...
                for s in sells:
                    ...
                    if _clean(b.get("broker")).lower() != _clean(s.get("broker")).lower():
                        continue
                    if _to_date(b.get("optie_exp_date")) == _to_date(s.get("optie_exp_date")):
                        continue
...
class ReviewWindow(QWidget):
    ...
    def render(self) -> None:
        ...
        self.lbl_meta.setText(
            "Regels: zelfde dag + asset + abs qty, tegengesteld, verschillende expiry, order_id/origin nog leeg."
        )
    ...
    def approve_current(self) -> None:
        ...
        QMessageBox.information(self, "Doorrol", f"{mode} met order_id={order_id} (Ids {p.left['Id']}, {p.right['Id']})")
        self.next_pair()
