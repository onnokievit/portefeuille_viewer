...
class OptiesOpenWebPilotTab(QWidget):
    ...
    def _publish_patch(self):
        patch = getattr(SNAPSHOT_STORE, "snapshot_opties_open_projection_v2_patch", None)
        if not patch:
            return
...
<html>
  ...
  <body>
    ...
    <script>
      ...
      function renderBody(){ const body=document.getElementById("tbody"); body.innerHTML=""; const rows=visibleRows(); rows.sort((a,b)=>compareRows(a,b,state.sortCol,state.sortDir)); for(const row of rows){ const tr=document.createElement("tr"); tr.id="r_"+String(row.row_id??""); for(const col of state.cols){ const td=document.createElement("td"); td.id="c_"+String(row.row_id??"")+"_"+col; const v=row[col]; td.textContent=fmt(v); if(isNum(v)) td.classList.add("num"); tr.appendChild(td);} body.appendChild(tr);} }
