...
CHART_ID = "cash_management_result"
...
<div class="app">
  <div class="top">
    <div>
      <div class="title">Resultaat per broker</div>
...
function fmt(v){
  ...
  const text = new Intl.NumberFormat("nl-NL", {maximumFractionDigits:0}).format(abs);
  return v < 0 ? `EUR (${text})` : `EUR ${text}`;
}
...
function buildLegend(brokers){
  ...
  total.className = "leg";
  total.innerHTML = `<span class="sw" style="border-top-color:#111111"></span><span>total result</span>`;
...
function render(payload){
  ...
  const plot = {l:42,r:90,t:14,b:52};
  ...
  for(let yVal = yMin; yVal <= yMax + step/2; yVal += step){
    ...
    line.setAttribute("stroke", yVal === 0 ? "#b5b5b5" : "#dfdfdf");
  ...
  if(totalSeries.length){
    ...
    path.setAttribute("stroke-width", String(Number(lineWidths["total_result"] || lineWidth)));
    if((lineStyles["total_result"] || "solid") === "dashed"){
      path.setAttribute("stroke-dasharray", "7 6");
    }
    svg.appendChild(path);
  }
  ...
  dates.forEach((d, idx)=>{
    ...
    hit.addEventListener("mousemove", (ev)=>{
      const brokerBits = brokers.map((broker)=>{
        const profit = d.brokers[broker]?.profit;
        return `${broker}: ${fmtPlain(profit)}`;
      }).join(" | ");
      tip.style.display = "block";
      tip.style.left = (ev.clientX + 12) + "px";
      tip.style.top = (ev.clientY + 12) + "px";
      tip.textContent = `${d.date} | totaal resultaat ${fmtPlain(d.total_profit)} | ${brokerBits}`;
    });
...
window.renderResultChart = render;
...
class _CashResultChartWorker(QThread):
...
class CashManagementResultChartDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Resultaat Chart")
    ...
    def refresh(self) -> None:
        if self._worker is not None and self._worker.isRunning():
            return
        self._status.setText("Resultaat chart laden...")
        self._refresh_button.setEnabled(False)
        self._worker = _CashResultChartWorker(self)
    ...
    def _on_payload_ready(self, payload: dict) -> None:
        ...
        if self._page_ready:
            ...
            self._status.setText("Geen resultaat chart data gevonden.")
    ...
    def _render_payload(self, payload: dict) -> None:
        ...
        js = f"window.renderResultChart({json.dumps(payload)});"
...
_CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE: CashManagementResultChartDialog | None = None


def open_cash_management_result_chart_dialog(parent=None) -> None:
    global _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE
    if _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE is None:
        _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE = CashManagementResultChartDialog(parent)
        _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE.destroyed.connect(
            lambda *_args: _clear_cash_management_result_chart_dialog_instance()
        )
    else:
        _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE.refresh()
    _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE.show()
    _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE.raise_()
    _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE.activateWindow()


def _clear_cash_management_result_chart_dialog_instance() -> None:
    global _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE
    _CASH_MANAGEMENT_RESULT_CHART_DIALOG_INSTANCE = None
