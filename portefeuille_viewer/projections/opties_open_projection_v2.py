...
class OptiesOpenProjectionV2:
    """Versioned projection for Open Opties (Live)."""

    name = "opties_open_v2"
    depends_on = {"aggregator_snapshot_load_open_opties_from_tx_live"}
    ...
    def recompute(self) -> None:
        df_src = getattr(SNAPSHOT_STORE, "aggregator_snapshot_load_open_opties_from_tx_live", None)
    ...
    def _prepare_df(df: pl.DataFrame) -> pl.DataFrame:
        ...
        rows = out.to_dicts()
        for r in rows:
            r["row_id"] = OptiesOpenProjectionV2._row_id_of(r)
